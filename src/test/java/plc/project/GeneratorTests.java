...
public class GeneratorTests {
    ...
    @ParameterizedTest(name = "{0}")
    @MethodSource
    void testFieldStatement(String test, Ast.Stmt.Field ast, String expected) {
        test(ast, expected);
    }

    private static Stream<Arguments> testFieldStatement() {
        return Stream.of(
    ...
    @ParameterizedTest(name = "{0}")
    @MethodSource
    void testMethodExpression(String test, Ast.Expr.Method ast, String expected) {
        test(ast, expected);
    }
    ...
    private static Stream<Arguments> testForStatement() {
        return Stream.of(
                Arguments.of("For",
                        ...
                        new Ast.Stmt.For(
                                "num",
                                init(new Ast.Expr.Access(Optional.empty(), "list"), ast -> ast.setVariable(new Environment.Variable("list", "list", Environment.Type.INTEGER, Environment.NIL))),
                                Arrays.asList(new Ast.Stmt.Expression(init(new Ast.Expr.Access(Optional.empty(), "stmt"), ast -> ast.setVariable(new Environment.Variable("stmt", "stmt", Environment.Type.NIL, Environment.NIL)))))
                        ),
                        String.join(System.lineSeparator(),
                                "for (int num : list) {",
